...
void PopupTemplate::create_file_save_as(EditorPopupManager* mgr, function<void(string)> on_save, string extension) {
	...
	mgr->add_popup("Save file as", [mgr, state]() mutable -> bool {
		...
		if (ImGui::InputText("##input", state.input_buffer.data(), state.input_buffer.size() + 1,
							 ImGuiInputTextFlags_CallbackResize, imgui_std_string_resize, &state.input_buffer)) {
		...
		if (ImGui::Button("Save")) {
			...
			if (file) {
				PopupTemplate::create_are_you_sure(mgr, "File \"" + fullpath + "\" already exists, overwrite?",
												   [&state]() { state.actually_wants_save = true; });
			} else {
