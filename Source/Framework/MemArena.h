...
public:
	...
	void free_top() { top_pointer = (uintptr_t)buffer + buffer_len; }

	void free_bottom() { bottom_pointer = (uintptr_t)buffer; }

	void* alloc_top(int64_t size);
	void* alloc_bottom(int64_t size);

	template <typename T> T* alloc_top_type(int count) { return (T*)alloc_top(count * sizeof(T)); }
	template <typename T> T* alloc_bottom_type(int count) { return (T*)alloc_bottom(count * sizeof(T)); }
	template <typename T> std::span<T> alloc_top_span(int count) {
		std::span<T> out(alloc_top_type<T>(count), count);
		return out;
	}
	template <typename T> std::span<T> alloc_bottom_span(int count) {
		std::span<T> out(alloc_bottom_type<T>(count), count);
		return out;
	}

	uintptr_t get_top_marker() { return top_pointer; }
	uintptr_t get_bottom_marker() { return bottom_pointer; }
	void free_top_to_marker(uintptr_t mark) { top_pointer = mark; }
	void free_bottom_to_marker(uintptr_t mark) { bottom_pointer = mark; }
