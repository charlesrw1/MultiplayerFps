...
public:
	...
	T* allocate(std::size_t n) { return static_cast<T*>(arena_->alloc(n * sizeof(T))); }
	...
	bool operator!=(const ArenaAllocator& other) const noexcept { return !(*this == other); }

	ArenaScope* arena_;
};

// Make Memory_Arena pointer accessible to other template instantiations
template <typename T, typename U> inline bool operator==(const ArenaAllocator<T>& a, const ArenaAllocator<U>& b) {
