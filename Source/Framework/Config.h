...
{
public:
	ConfigVar(const char* name, const char* value, int flags, const char* description, float min = -1.f,
			  float max = 1.f);
	...
	float get_max_val() const { return ptr->maxVal; }
	float get_min_val() const { return ptr->minVal; }
	const char* get_desc() const { return ptr->description; }
	...
	void force_set_has_changed() { ptr->flags |= CVAR_CHANGED; }
...
public:
	...
	void set_output_pipe(std::string* ptr) { pipe_output_to_this = ptr; }
...
{
public:
	Auto_Engine_Cmd(const char* name, Engine_Cmd_Function cmd) { Cmd_Manager::get()->add_command(name, cmd); }
};
...
{
	AddToDebugMenu(const char* name, void (*func)()) { Debug_Interface::get()->add_hook(name, func); }
};
