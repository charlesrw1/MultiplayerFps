...
template <typename T> using arena_vec = std::vector<T, ArenaAllocator<T>>;

template <typename Key, typename T>
using arena_map =
	std::unordered_map<Key, T, std::hash<Key>, std::equal_to<Key>, ArenaAllocator<std::pair<const Key, T>>>;
