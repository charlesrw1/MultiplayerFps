...
struct EnumIntPair
{
	EnumIntPair(const char* n, const char* d, int64_t v) : name(n), display_name(d), value(v) {}
	EnumIntPair() {}
	const char* name = "";
...
struct EnumIterator
{
	...
	bool operator!=(const EnumIterator& other) { return index != other.index; }
...
#define NEWENUM(type, inttype)                                                                                         \
	...
	template <> struct ::EnumTrait<type>                                                                               \
