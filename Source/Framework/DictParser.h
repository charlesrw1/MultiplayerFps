...
public:
	...
	bool check_item_start(const StringView& tok) { return tok.cmp("{"); }
	bool check_item_end(const StringView& tok) { return tok.cmp("}"); }

	bool check_list_start(const StringView& tok) { return tok.cmp("["); }
	bool expect_list_start() {
		StringView tok;
		read_string(tok);
		return check_list_start(tok);
	}
	bool check_list_end(const StringView& tok) { return tok.cmp("]"); }
	...
	bool read_string(StringView& str) { return read_next_token(str); }
	...
	bool is_eof() { return read_ptr >= buffer_size; }

	bool has_read_error() { return had_error; }

	bool expect_no_more_tokens();

	template <typename FUNCTOR> bool read_list_and_apply_functor(FUNCTOR&& f) {
	...
	int get_marker() { return read_ptr; }
