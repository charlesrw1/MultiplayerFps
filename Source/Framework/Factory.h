 ...
 */
template <typename K, typename T> class Factory
{
private:
	 ...
	 */
	template <typename S> static T* createObject() { return new S(); }

public:
	const std::unordered_map<K, CreateObjectFunc>& get_object_creator() const { return mObjectCreator; }
	 ...
	 */
	template <typename S> void registerClass(K id) {
	 ...
	 */
	bool hasClass(K id) { return mObjectCreator.find(id) != mObjectCreator.end(); }
