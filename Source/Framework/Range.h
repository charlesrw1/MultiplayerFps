...
public:
	...
	template <typename T> Range(const std::vector<T>& v) : endIdx((int)v.size()) {}
	...
	struct Iterator
	{
		...
		bool operator!=(const Iterator& o) const { return index < endIdx; }
	};
...
template <typename T> class IPairs
{
public:
	...
	struct Iterator
	{
		...
		Data operator*() const { return Data{index, &ar.at(index)}; }
		Iterator& operator++() {
			index += 1;
			return *this;
		}
		bool operator!=(const Iterator& o) const { return index < (int)ar.size(); }
