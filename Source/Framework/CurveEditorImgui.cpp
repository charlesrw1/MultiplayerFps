...
void CurveEditorImgui::draw_editor_space() {
	...
	const ImVec2 grid = screenspace_to_grid(ImVec2(valx, valy));
	{
		...
		for (float x = canvas_pos.x - fmod(grid_offset.x / base_scale.x * scale.x, grid_size.x);
			...
			for (int i = 1; i < subdivisions.x; i++) {
				drawlist->AddLine(ImVec2(x + i * subgrid_size.x, canvas_pos.y),
								  ImVec2(x + i * subgrid_size.x, canvas_pos.y + canvas_size.y), col_subdiv);
			}
			i++;
		}
	}
	...
	for (int curve_index = 0; curve_index < curves.size(); curve_index++) {
		...
		for (int i = 0; i < (int)points.size() - 1; i++) {
			...
			if (point.type == CurvePointType::Constant) {
				...
				for (int j = 0; j < BEZIER_CURVE_SUBDIV - 1; j++) {
					drawlist->AddLine(grid_to_screenspace(ImVec2(pointsout[j].x, pointsout[j].y)),
									  grid_to_screenspace(ImVec2(pointsout[j + 1].x, pointsout[j + 1].y)), curvecol);
				}
			}
		}
		...
		for (int i = 0; i < points.size(); i++) {
			...
			const bool draw_tangents = is_curve_selected(curve_index) && (point.type == CurvePointType::SplitTangents ||
	...
	auto draw_event = [&](int ITEM_INDEX) -> bool {
		...
		bool return_hovered = false;
		{
			...
			if (!is_dragging_selected &&
				ImRect(ImVec2(x_start_ss, y_start_ss), ImVec2(x_end_ss, y_end_ss)).Contains(ImGui::GetMousePos())) {
				if (ImGui::GetIO().MouseClicked[0]) {
					set_selected_event(ITEM_INDEX, true);
			...
			drawlist->AddText(ImVec2(x_start_ss + 6.0, y_start_ss + 1.0), COLOR_WHITE.to_uint(),
							  item->get_name().c_str());
		}
	...
	if (ImGui::BeginPopup("point_popup")) {
		...
		if (!is_selected_curve_valid()) {
			...
			auto enum_ =
				EnumTrait<CurvePointType>::StaticEnumType.find_for_value((int)points.at(dragged_point_index).type);
			ASSERT(enum_);
	...
	if (ImGui::GetIO().MouseClicked[1] && is_window_focused_and_mouse_in_region && is_selecting_a_curve() &&
		dragged_point_index == -1 /*hack*/) {
...

	{
		...
		if (origin_ss.x - BASE_SCREENPOS.x >= 0) {
			drawlist->AddRectFilled(BASE_SCREENPOS, ImVec2(origin_ss.x, BASE_SCREENPOS.y + WINDOW_SIZE.y),
									edges.to_uint());
		}
	...
	// draw y after drawing mask boxes
	{
		...
		for (float y = canvas_pos.y - fmod(grid_offset.y / base_scale.y * scale.y, grid_size.y);
			...
			for (int i = 1; i < subdivisions.y; i++) {
				drawlist->AddLine(ImVec2(canvas_pos.x, y + i * subgrid_size.y),
								  ImVec2(canvas_pos.x + canvas_size.x, y + i * subgrid_size.y), col_subdiv);
			}
			i++;
		}
	}
	...
	if (ImRect(BASE_SCREENPOS, ImVec2(BASE_SCREENPOS.x + WINDOW_SIZE.x, BASE_SCREENPOS.y + TIMELINE_HEIGHT))
			.Contains(ImGui::GetMousePos()) &&
	...
	}
	{
		...
		drawlist->AddLine(ImVec2(pos_of_scrubber.x, BASE_SCREENPOS.y),
						  ImVec2(pos_of_scrubber.x, BASE_SCREENPOS.y + WINDOW_SIZE.y), IM_COL32(27, 145, 247, 255),
						  2.0);
		drawlist->AddRectFilled(ImVec2(pos_of_scrubber.x - off, BASE_SCREENPOS.y),
								...
								IM_COL32(27, 145, 247, 255), 6.0);
		drawlist->AddText(ImVec2(pos_of_scrubber.x - off + 2.0, BASE_SCREENPOS.y), IM_COL32(255, 255, 255, 255),
						  time_str);
	}

	if (!dragging_scrubber && !is_dragging_selected && ImGui::IsWindowFocused() && !is_any_event_hovered &&
...
void CurveEditorImgui::draw() {
		...
		//
		{
			...
			if (ImGui::BeginTable("flags", 1, inner_flags)) {
				...
				for (int row_n = 0; row_n < curves.size(); row_n++) {
					...
					ImGuiSelectableFlags selectable_flags =
						ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowItemOverlap;
					if (ImGui::Selectable("##selectednode", is_curve_selected(row_n), selectable_flags,
										  ImVec2(0, 24))) {
	...
	if (ImGui::BeginPopup("curve_popup")) {
		if (!is_selected_curve_valid()) {
			...
			ImGui::InputText("##name", (char*)curve.name.data(), curve.name.size() + 1 /*null terminator, FIXME?*/,
							 ImGuiInputTextFlags_CallbackResize, imgui_std_string_resize, &curve.name);
...
void BakedCurve::bake_from(const std::vector<EditingCurve>& curves, float max_time, float min_y,
	...
	for (int curve_idx = 0; curve_idx < curves.size(); curve_idx++) {
		...
		std::sort(outsorted.begin(), outsorted.end(),
				  [](const auto& p1, const auto& p2) -> bool { return p1.time < p2.time; });
		for (int i = 0; i < total_keyframes; i++) {
			...
			auto find = std::lower_bound(outsorted.begin(), outsorted.end(), time,
										 [](const outstruct& p1, float time) -> bool { return p1.time < time; });
