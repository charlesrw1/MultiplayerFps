...
template <typename T> struct hash_set_iterator
{
	...
	bool operator!=(const hash_set_iterator& other) { return index != other.index; }
	T* operator*();
...
public:
	...
	hash_set_iterator<T> begin() { return hash_set_iterator<T>(this, 0); }
	hash_set_iterator<T> end() { return hash_set_iterator<T>(this, items.size()); }
	hash_set_iterator<const T> begin() const { return hash_set_iterator<const T>(this, 0); }
	hash_set_iterator<const T> end() const { return hash_set_iterator<const T>(this, items.size()); }
...
template <typename T> inline T* hash_set_iterator<T>::operator*() {
	return parent->items.at(index).ptr;
}

template <typename T> inline hash_set_iterator<T>::hash_set_iterator(const hash_set<ActualType>* parent, int i) {
	this->parent = parent;
	index = i;
	advance();
}

template <typename T> inline void hash_set_iterator<T>::advance() {
