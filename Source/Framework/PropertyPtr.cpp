...
ClassPropPtr::Iterator::Iterator(ClassBase* obj) : obj(obj) {
	info = &obj->get_type();
	advance();
}
ClassPropPtr::Iterator::Iterator(const ClassTypeInfo* ti) : obj(nullptr) {
	info = ti;
	advance();
}
...
bool PropertyPtr::is_enum() const {
	return core_type_id::Enum8 == get_type() || core_type_id::Enum16 == get_type() ||
		   core_type_id::Enum32 == get_type();
}
