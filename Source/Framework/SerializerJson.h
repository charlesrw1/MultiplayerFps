...
public:
	...
	MakePath make_path(const ClassBase* toobj) final { return "x" + std::to_string(uintptr_t(toobj)); }
	std::string make_type_name(ClassBase* obj) final { return obj->get_type().classname; }
...
public:
	WriteSerializerBackendJson(const char* debug_tag, IMakePathForObject& pathmaker, ClassBase& obj_to_serialize,
							   bool serialize_single_object = false);
...
public:
	...
	ClassBase* get_root_obj() { return rootobj; }
...
private:
	...
	bool serialize_class_shared(opt<const char*> tag, const ClassTypeInfo& info, ClassBase*& ptr,
								bool is_only_reference);
