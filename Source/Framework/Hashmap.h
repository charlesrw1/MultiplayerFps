...
template <typename T> struct hash_map_iterator
{
...
public:
	...
	hash_map_iterator<T> begin() const { return hash_map_iterator<T>(this, 0); }
	hash_map_iterator<T> end() const { return hash_map_iterator<T>(items.size()); }
...
template <typename T> inline T hash_map_iterator<T>::operator*() {
	return parent->items[index].data;
}

template <typename T> inline hash_map_iterator<T>::hash_map_iterator(const hash_map<T>* parent, uint64_t i) {
	this->parent = parent;
	index = i;
	advance();
}

template <typename T> inline void hash_map_iterator<T>::advance() {
