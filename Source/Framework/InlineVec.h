...
template <typename T, uint32_t INLINE_COUNT> class InlineVec
{
public:
	...
	T* data() { return is_allocated_() ? heap.ptr : fixed.inline_; }
	const T* data() const { return is_allocated_() ? heap.ptr : fixed.inline_; }
	std::span<T> get_span() { return std::span<T>(data(), size()); }
	std::span<const T> get_span() const { return std::span<const T>(data(), size()); }
	...
	void resize(SIZE_TYPE newcount) { resize(newcount, {}); }
	void resize(SIZE_TYPE newcount, const T& value);
	void clear() { resize(0); }

private:
	void set_size_(SIZE_TYPE s) { count_ = (s << 1) | (count_ & 1); }
...
template <typename T, uint32_t INLINE_COUNT> inline InlineVec<T, INLINE_COUNT>::InlineVec(const InlineVec& other) {
	reserve(other.size());
...
template <typename T, uint32_t INLINE_COUNT> inline void InlineVec<T, INLINE_COUNT>::reserve(SIZE_TYPE reserve_size) {
	if (capacity_() < reserve_size) {
		expand_capacity_(reserve_size);
	}
}

template <typename T, uint32_t INLINE_COUNT> inline void InlineVec<T, INLINE_COUNT>::push_back(T&& val) {
...
template <typename T, uint32_t INLINE_COUNT> inline void InlineVec<T, INLINE_COUNT>::push_back(const T& val) {
	SIZE_TYPE cap = capacity_();
	SIZE_TYPE curcount = size();
	if (curcount + 1 > cap) {
		// need to increase capacity
...
template <typename T, uint32_t INLINE_COUNT> inline void InlineVec<T, INLINE_COUNT>::destroy_(T* start, int count) {
	if constexpr (!std::is_trivially_destructible<T>()) {
		std::destroy(start, start + count);
	}
}
template <typename T, uint32_t INLINE_COUNT> inline InlineVec<T, INLINE_COUNT>::InlineVec() {}
template <typename T, uint32_t INLINE_COUNT> inline InlineVec<T, INLINE_COUNT>::~InlineVec() {
	destroy_(data(), size());
}
template <typename T, uint32_t INLINE_COUNT>
