...
void GradientEditorImgui::draw_gradient_bar(const Gradient& gradient, ImVec2 pos, ImVec2 size)
{
...
bool GradientEditorImgui::draw_preview(const char* label, const Gradient& gradient, float width, float height)
{
	ImGui::PushID(label);
	if (width <= 0.f)
	...
	draw_gradient_bar(gradient, pos, size);

	bool clicked = ImGui::InvisibleButton("##gradient_preview", size);
	ImGui::PopID();
	return clicked;
}

bool GradientEditorImgui::draw_editor(const char* label, Gradient& gradient, float width, float height)
{
	bool modified = false;
	ImGui::PushID(label);
