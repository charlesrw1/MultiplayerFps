...
struct Serializer
{
	...
	template <typename T> bool serialize_struct(const char* tag, T& t);
	template <typename T> bool serialize_class(const char* tag, T*& ptr);			// owning
	template <typename T> bool serialize_class_reference(const char* tag, T*& ptr); // non owning
	template <typename T> void serialize_struct_ar(T& t);
	template <typename T> void serialize_class_ar(T*& ptr);			  // owning
	template <typename T> void serialize_class_reference_ar(T*& ptr); // non owning
...
template <typename T> inline bool Serializer::serialize_struct(const char* tag, T& t) {
	return serialize_struct(tag, T::StructType, &t);
}

template <typename T> inline bool Serializer::serialize_class(const char* tag, T*& ptr) {
	ClassBase* p = ptr;
...
template <typename T> inline bool Serializer::serialize_class_reference(const char* tag, T*& ptr) {
	ClassBase* p = ptr;
	bool res = serialize_class_reference(tag, T::StaticType, p);
	ptr = (T*)p;
	return res;
}
template <typename T> inline void Serializer::serialize_struct_ar(T& t) {
	serialize_struct_ar(T::StructType, &t);
}

template <typename T> inline void Serializer::serialize_class_ar(T*& ptr) {
	ClassBase* p = ptr;
	serialize_class_ar(T::StaticType, p);
	ptr = (T*)p;
}

template <typename T> inline void Serializer::serialize_class_reference_ar(T*& ptr) {
	ClassBase* p = ptr;
