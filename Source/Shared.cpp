...
extern int col_iters;
extern bool col_closest;
extern bool col_response;
extern bool gravity;
extern bool new_physics;
extern float move_speed_player;
...
void PlayerPhysics::GroundMove()
{
	...
	player->angles.x = inp.view_angles.y;
}
void PlayerPhysics::AirMove()
{

}
static void UpdatePlayer(PlayerState* player, MoveCommand cmd, double dt);
void PlayerPhysics::Run(void(*trace_capsule)(ColliderCastResult*, const Capsule&, bool), PlayerState* p, MoveCommand cmd, float dt)
{
	if (!p)
		return;
	deltat = dt;
