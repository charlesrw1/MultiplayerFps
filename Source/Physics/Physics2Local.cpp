...
bool PhysicsManager::sweep_capsule(world_query_result& out, const vertical_capsule_def_t& capsule,
								   const glm::vec3& start, const glm::vec3& dir, float length, uint32_t channel_mask,
...
bool PhysicsManager::sweep_sphere(world_query_result& out, float radius, const glm::vec3& start, const glm::vec3& dir,
								  float length, uint32_t channel_mask, const TraceIgnoreVec* ignore) {
	auto geom = physx::PxSphereGeometry(radius);
	return impl->sweep_shared(out, geom, start, dir, length, ignore, channel_mask);
}
bool PhysicsManager::capsule_is_overlapped(overlap_query_result& out, const vertical_capsule_def_t& capsule,
										   const glm::vec3& start, uint32_t mask) {
	auto geom = physx::PxCapsuleGeometry(capsule.radius, capsule.half_height);
	return impl->overlap_shared(out, geom, start, mask);
}
bool PhysicsManager::sphere_is_overlapped(overlap_query_result& out, float radius, const glm::vec3& start,
										  uint32_t mask) {
	auto geom = physx::PxSphereGeometry(radius);
	return impl->overlap_shared(out, geom, start, mask);
}
...
public:
	...
	virtual PxQueryHitType::Enum postFilter(const PxFilterData& filterData, const PxQueryHit& hit, const PxShape* shape,
...
bool PhysicsManager::trace_ray(world_query_result& out, const glm::vec3& start, const glm::vec3& end,
							   const TraceIgnoreVec* ignore, uint32_t mask) {
	float length = glm::length(end - start);
	glm::vec3 dir = (end - start) / length;
	return trace_ray(out, start, dir, length, ignore, mask);
}
bool PhysicsManager::trace_ray(world_query_result& out, const glm::vec3& start, const glm::vec3& dir, float length,
	...
	bool status = impl->scene->raycast(glm_to_physx(start), glm_to_physx(dir), length, hit, PxHitFlag::eDEFAULT, filter,
...
bool PhysicsManImpl::sweep_shared(world_query_result& out, physx::PxGeometry& geom, const glm::vec3& start,
								  const glm::vec3& dir, float length, const TraceIgnoreVec* ignore,
	...
	bool status = scene->sweep(geom, local, glm_to_physx(dir), length, sweep,
							   PxHitFlag::eDEFAULT | PxHitFlag::eMTD /* for initial overlaps */, filter,
							   (ignore) ? &query_filter : nullptr);
...
bool PhysicsManImpl::load_physics_into_shape(BinaryReader& reader, physics_shape_def& def) {
	if (def.shape != ShapeType_e::ConvexShape && def.shape != ShapeType_e::MeshShape)
		return true;
...
bool PhysicsManImpl::overlap_shared(overlap_query_result& out, physx::PxGeometry& geom, const glm::vec3& start,
									uint32_t channel_mask) {
	// MyPhysicsQueryFilter query_filter(ignore);
	PxQueryFilterData filter;
	filter.data.word0 = channel_mask;
