...
glm::mat4 integrate_spring_bone(float dt, const SpringBoneParams& p, const glm::mat4& worldRestMat,
...
void SpringBones::update(float dt, const Pose& pose, std::vector<glm::mat4>& cached_bonemats, const MSkeleton* skel,
	...
	for (int i = 0; i < (int)defs.size(); i++) {
		...
		const glm::mat4 worldResultMat = integrate_spring_bone(dt, p, worldRestMat, worldParentPos, state);
