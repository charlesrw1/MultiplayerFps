...
// why tf U ARENT WORKING???!
typedef void* ImNodesContext_Ptr;
...
ImNodesContext_Ptr CreateContext();
void            DestroyContext(ImNodesContext_Ptr ctx); // NULL = destroy current context
ImNodesContext_Ptr GetCurrentContext();
void            SetCurrentContext(ImNodesContext_Ptr ctx);
...
void Link(int id, int start_attribute_id, int end_attribute_id, bool no_curve = false);
