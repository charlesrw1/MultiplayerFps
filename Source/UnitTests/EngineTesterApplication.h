...
template<typename T>
inline T negative_modulo(T x, T mod_)
{
...
public:
	...
	REF void add(CamPathPoints points) {
	...
	REF void update() {
		...
		if (path.points.size() == 1) {
			...
			scene_cam->get_owner()->set_ws_position_rotation(
				p0.pos,
				p0.rot
			);
		}
			...
			scene_cam->get_owner()->set_ws_position_rotation(
				glm::mix(glm::vec3(p0.pos), glm::vec3(p1.pos), frac),
