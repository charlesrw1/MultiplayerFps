...
#define checkTrue(code) checkTrueComment(code, "expected true")
...
public:
	...
	void add_test(const char* category, const test_func_t func) { allTests.push_back({category, func}); }
...
{
	AutoTestCaseAdd(const char* name, test_func_t f) { ProgramTester::get().add_test(name, f); }
};
