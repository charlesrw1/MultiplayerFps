...
static Animation_Set* load_animation_set_for_gltf_skin(cgltf_data* data, cgltf_skin* skin) {
	...
	for (int a = 0; a < data->animations_count; a++) {
		...
		for (int c = 0; c < gltf_anim->channels_count; c++) {
			...
			if (node_to_index.find(node_of_target) ==
				node_to_index.end()) // channel is associated with a different armature or something
				continue;
...
ProcessMeshOutput ModelCompileHelper::process_mesh(ModelCompileData& mcd, const SkeletonCompileData* scd,
	...
	for (int i = 0; i < mcd.lod_where.size(); i++) {
		...
		for (int MESH_NODE_IDX = 0; MESH_NODE_IDX < NUM_NODES_PRE_ADD; MESH_NODE_IDX++) {
			...
			const uint32_t MAX_VERTICIES_PER_SUBMESH =
				UINT16_MAX - 1000; /*just give me some buffer room just in case...*/
			if (mesh.submesh.vertex_count >= MAX_VERTICIES_PER_SUBMESH) {
				...
				auto find_or_append = [&](uint32_t OLD_INDEX) -> uint32_t {
					...
					new_vertex_to_old_vertex.insert(
						{OLD_INDEX, added_verticies}); // insert mapping from new to old vertex
					added_verticies++;				   // increment added verticies, check this too
					mcd.verticies.push_back(
						mcd.verticies.at(mesh.submesh.base_vertex + OLD_INDEX)); // add the old vertex
...
std::vector<std::string> get_imported_models(const ModelDefData& def) {
...

		{
			...
			if (find != data.bone_reparent.end()) {
				...
				if (parent_to_this == -1) {
					...
					auto parentinv =
						glm::inverse(glm::mat4(parent.posematrix)); // inverse of the parents world transform
...
std::vector<BonePoseMask> get_bone_masks(const std::vector<int>& FINAL_to_LOAD_bone,
...
ProcessNodesAndMeshOutput process_nodes_and_mesh(cgltf_data* data, const SkeletonCompileData* scd,
	...
	// Okay im pretty stupid, theres some indexing bug when i do optimizeVertexFetch with auto gened lods and I dont
	// want to fix it, just do the stupid thing and do the optmize part for the imported meshes first, then the auto-gen
	// after. (auto-gen'ing happens in process_mesh()) so autogen mesh will simply used the remapped buffer to start
	// with, EZ!
...
void add_data_to_vertex_shared(const FATVertex& v, const glm::mat4& transform, const glm::mat3& normal_tr,
							   ModelVertex& mv) {
	mv.pos = v.position;
...
#include "physx/cooking/PxCooking.h"
FinalPhysicsData create_final_physics_data(const std::vector<std::string>& final_mat_names,
...
Submesh make_final_submesh_from_existing(const bool has_skeleton, const Submesh& in, FinalModelData& finalmod,
...
FinalModelData create_final_model_data(const FinalSkeletonOutput* skel, const std::vector<std::string>& final_mat_names,
...
bool write_out_compilied_model(const std::string& gamepath, const FinalModelData* model,
