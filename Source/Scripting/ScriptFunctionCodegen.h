...
public:
	...
	// in radians
	REF static glm::vec3 angles_to_vector(float pitch, float yaw) { return AnglesToVector(pitch, yaw); }
	REF static float fmod(float f, float m) { return std::fmodf(f, m); }
	REF static float atan2f(float y, float x) { return std::atan2f(y, x); }
	...
	REF static float pow(float x, float p) { return std::powf(x, p); }
...
template <typename T> ClassTypeInfo::CreateObjectFunc get_allocate_script_impl_internal() {
