...
public:
	static double get_bytes_per_pixel(GraphicsTextureFormat fmt) {
		switch (fmt) {
		case GraphicsTextureFormat::r8:
			return 1;
			break;
		case GraphicsTextureFormat::rg8:
			return 2;
			break;
		...
		case GraphicsTextureFormat::rgba8:
			return 4;
			break;
		case GraphicsTextureFormat::r16f:
			return 2;
			break;
		...
		case GraphicsTextureFormat::rgba16f:
			return 8;
			break;
		case GraphicsTextureFormat::r32f:
			return 4;
			break;
		...
		case GraphicsTextureFormat::r11f_g11f_b10f:
			return 4;
			break;
		case GraphicsTextureFormat::rgba16_snorm:
			return 8;
			break;
		default:
			break;
	...
	static GLenum to_format(GraphicsTextureFormat fmt) {
		...
		case GraphicsTextureFormat::bc1:
			...
			break;
		case GraphicsTextureFormat::bc1_srgb:
	...
	static GLenum filter_to_gl(GraphicsFilterType type) {
		switch (type) {
		case GraphicsFilterType::Linear:
			return GL_LINEAR;
			break;
		case GraphicsFilterType::Nearest:
			return GL_NEAREST;
			break;
		case GraphicsFilterType::MipmapLinear:
	...
	static GLenum wrap_to_gl(GraphicsTextureEdge type) {
		switch (type) {
		case GraphicsTextureEdge::Repeat:
			return GL_REPEAT;
			break;
		case GraphicsTextureEdge::Clamp:
...
public:
	...
	void sub_upload(const void* data, int size, int ofs) override { glNamedBufferSubData(id, ofs, size, data); }
	uint32_t get_internal_handle() override { return id; }
...
public:
	...
	static GLenum get_type_enum(GraphicsVertexAttribType type) {
		using gvat = GraphicsVertexAttribType;
		switch (type) {
		case GraphicsVertexAttribType::u8:
			return GL_UNSIGNED_BYTE;
			break;
		case GraphicsVertexAttribType::u16:
			return GL_UNSIGNED_SHORT;
			break;
		case GraphicsVertexAttribType::i8:
			return GL_BYTE;
			break;
		case GraphicsVertexAttribType::i16:
			return GL_SHORT;
			break;
		case GraphicsVertexAttribType::u8_normalized:
			return GL_UNSIGNED_BYTE;
			break;
		case GraphicsVertexAttribType::u16_normalized:
			return GL_UNSIGNED_SHORT;
			break;
		case GraphicsVertexAttribType::i8_normalized:
			return GL_BYTE;
			break;
		case GraphicsVertexAttribType::i16_normalized:
			return GL_SHORT;
			break;
		case GraphicsVertexAttribType::float32:
			return GL_FLOAT;
			break;
		default:
			break;
...
{
public:
	fbohandle get_next_fbo() { return shared_framebuffer; }
	...
	void set_framebuffer_with_info(const RenderPassState& state, fbohandle framebuffer_to_use, int& min_width,
								   int& min_height) {
		...
		for (int i = 0; i < state.color_infos.size(); i++) {
			...
			if (info.layer == -1)
				...
				glNamedFramebufferTextureLayer(framebuffer_to_use, GL_COLOR_ATTACHMENT0 + i, texture->id, info.mip,
											   info.layer);
		...
		if (state.depth_info) {
			...
			if (state.depth_layer == -1)
				...
				glNamedFramebufferTextureLayer(framebuffer_to_use, GL_DEPTH_ATTACHMENT, texture->id, 0,
											   state.depth_layer);
			min_width = std::min(texture->width, min_width);
			min_height = std::min(texture->height, min_height);
		} else {
	...
	void blit_textures(const GraphicsBlitInfo& info) override {
		...
		glBlitNamedFramebuffer(src_fbo, dest_fbo, info.src.x, info.src.y, info.src.w, info.src.h, info.dest.x,
							   info.dest.y, info.dest.w, info.dest.h, GL_COLOR_BUFFER_BIT,
							   OpenGLTextureImpl::filter_to_gl(info.filter));
	}

	IGraphicsTexture* get_swapchain_texture() override { return &swapchain_fake_img; }

	IGraphicsTexture* create_texture(const CreateTextureArgs& args) override { return new OpenGLTextureImpl(args); }
	IGraphicsBuffer* create_buffer(const CreateBufferArgs& args) override { return new OpenGLBufferImpl(args); }
...
void OpenglDataStatic::dump_to_disk(std::string str) {
	...
	for (auto t : all_textures) {
		output +=
			string_format("texture,%s,%d\n", graphics_texture_format_to_str(t->get_texture_format()), t->mem_usage);
	}
