...
void MaterialImpl::load_instance(MaterialInstance* self, IFile* file, IAssetLoadingInterface* loading) {
	...
	try {
		...
		auto parent = g_assets.find_sync_sptr<MaterialInstance>(
			parent_mat); // loading->load_asset(&MaterialInstance::StaticType, parent_mat);
...
void MasterMaterialImpl::load_from_file(const std::string& fullpath, IFile* file, IAssetLoadingInterface* loading) {
	...
	// i have another layer beneath this  (in Prog_Man) that compiles the glsl to a platform binary (which is cached),
	// but that layer depends on dynamic usage state like if its an animator object, editor mode, etc. so it has to be
	// done there.
