...
// --------------------------------------------------------------------------------------------------------------------
template <typename Atom> class BufferGl
{
public:
	BufferGl(bool _cpuUpdates)
		: mLockManager(_cpuUpdates), mBufferContents(), mName(), mTarget(),
		  mBufferStorage(BufferStorage::SystemMemory) {}

	~BufferGl() { Destroy(); }

	bool Create(BufferStorage _storage, GLenum _target, GLuint _atomCount, GLbitfield _createFlags,
		...
		case BufferStorage::PersistentlyMappedBuffer: {
			...
			mBufferContents =
				reinterpret_cast<Atom*>(glMapBufferRange(mTarget, 0, sizeof(Atom) * _atomCount, _mapFlags));
			if (!mBufferContents) {
	...
	void WaitForLockedRange(size_t _lockBegin, size_t _lockLength) {
		mLockManager.WaitForLockedRange(_lockBegin, _lockLength);
	}
	...
	void BindBufferRange(GLuint _index, GLsizeiptr _head, GLsizeiptr _atomCount) {
		glBindBufferRange(mTarget, _index, mName, _head * sizeof(Atom), _atomCount * sizeof(Atom));
	}
...
// --------------------------------------------------------------------------------------------------------------------
template <typename Atom> class CircularBuffer
{
public:
	CircularBuffer(bool _cpuUpdates = true) : mBuffer(_cpuUpdates) {}
