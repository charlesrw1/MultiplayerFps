...
//
// The ragdoll component also has to be initialized with multiple PhysicsBodies and Joints. These PhysicsBodies
// themselves *arent* parented to bones, they are "free" entities. but they are linked to a bone and can also have a
// local space transform to the bone
//
...
// now clearly thats a lot of writing for just 2 bones. youll want to make some functions to make the process of adding
// the bodies/joints easier.
...
public:
	...
	int get_num_bodies() const { return bodies.size(); }
	int get_body_index(int i) { return bodies.at(i).bone_index; }
