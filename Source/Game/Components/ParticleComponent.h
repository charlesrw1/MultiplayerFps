...
public:
	...
	TrailComponent() { set_call_init_in_editor(true); }
	void start() final;
	void stop() final;
	...
	REF void set_material(MaterialInstance* m) { this->material = m; }
...
public:
	...
	BeamComponent() { set_call_init_in_editor(true); }
	void start() final;
	void stop() final;
	void on_changed_transform() final;
	void on_sync_render_data() final;
	...
	REF void set_vars(bool gravity, bool use_target, float stiff, float damp, float width, MaterialInstance* material,
					  int sample_points) {
	...
	REF void set_target_pos(glm::vec3 p) { this->world_pos_target = p; }
	REF void reset();
