...
public:
	...
	REF void print(std::string s) {
		sys_print(Warning, "%s\n", s.c_str());
	}

	REF void set_rate(float f) { 
		continious_rate = f; }
	REF void set_burst(float time, float min, float max) { 
		burst_time = time; burst_min = min; burst_max = max; }

	REF void set_gravity_drag(float g, float d) { 
		gravity = g; drag = d; }
	REF void set_size(float min, float max, float decay) { 
		size_min = min; size_max = max; size_decay = decay; }
	REF void set_color(lColor start, lColor end) {
		start_color = start.to_color32(); end_color = end.to_color32();
	}
