...
private:
	...
	// returned for get_ws_X calls, invalidated when local space moves or parent moves
	glm::mat4 cached_world_transform = glm::mat4(1);
	bool world_transform_is_dirty = true;
	// if true, then local space is considered the world space transform, even if a parent exists
