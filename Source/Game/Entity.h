...
public:
	...
	REF void destroy();
	template <typename T> T* get_component() const { return (T*)get_component(&T::StaticType); }
	REFLECT(lua_generic)
...
private:
	...
	glm::mat4 cached_world_transform =
		glm::mat4(1); // returned for get_ws_X calls, invalidated when local space moves or parent moves
#ifdef EDITOR_BUILD
	...
	bool is_top_level =
		false; // if true, then local space is considered the world space transform, even if a parent exists
...
template <typename T> inline T* Entity::create_component() {
...
template <typename T> inline T* Entity::create_entity_with_component() {
