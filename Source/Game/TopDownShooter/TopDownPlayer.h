...
private:
	...
	float eval_mult(float t) {
		if (t <= fade_in_time)
			return t / fade_in_time;
		else if (t >= total_time - fade_out_time)
			return 1.0 - ((t - (total_time - fade_out_time)) / fade_out_time);
		return 1.0;
	}
...
public:
	...
	void shoot_gun() {
		...
		if (shoot_cooldown <= 0.0) {
			...
			for (int i = 0; i < count; i++) {
				...
				//
				// pc->get_owner()->set_ws_transform(get_ws_position() + glm::vec3(0, 0.5, 0),
				// glm::quat_cast(rotationMatrix), pc->get_owner()->get_ls_scale());
			}
	...
	glm::vec3 get_front_dir() const { return AnglesToVector(view_angles.x, view_angles.y); }
	int var = 0;
