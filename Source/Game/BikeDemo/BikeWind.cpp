...
// ============================================================
// WindSystem global instance
// ============================================================

WindSystem g_wind;

// ============================================================
// Per-streak property ranges
// ============================================================

static float radius_min   =  6.f;
static float radius_max   = 34.f;
static float len_near     =  7.f;
static float len_far      = 19.f;
static float width_near   =  0.055f;
static float width_far    =  0.19f;
static float alpha_near   = 200.f;
static float alpha_far    =  70.f;
...
static float wind_cam_fade_dist = 8.f;

// Flow field
static float flow_lat_scale   = 0.23f;
static float flow_vert_scale  = 0.35f;
static float flow_freq        = 0.07f;
static float flow_time_speed  = 0.09f;
static float flow_turb_scale  = 0.28f;
static float flow_turb_freq   = 0.28f;
...
void WindSystem::update(BikeObject* bike, glm::vec3 camera_pos)
{
	...
	// Flow field — two Perlin octaves: large-scale lateral+vertical + small turbulence
	...
	auto spawn_streak = [&](WindLine& wl, bool stagger) {
	...
	for (int i = 0; i < WIND_LINE_COUNT; i++) {
		...
		for (int s = 0; s <= WIND_SEGS; s++) {
			const float t_     = (float)s / WIND_SEGS;
			const float env_w  = glm::pow(glm::sin(glm::pi<float>() * t_), 0.55f);
			const float env_a  = glm::pow(glm::sin(glm::pi<float>() * t_), 0.40f);
...
static void bike_wind_debug()
{
	...
	{
		ImGui::DragFloat("ambient_temp", &g_wind.ambient_temp, 0.5f, -5.f, 45.f);
		ImGui::DragFloat("sun_exposure", &g_wind.sun_exposure, 0.02f, 0.f,  1.f);
	}
	ImGui::SeparatorText("Wind");
	{
		ImGui::DragFloat3("wind_direction",    &g_wind.wind_direction.x, 0.05f);
		ImGui::DragFloat ("wind_speed",        &g_wind.wind_speed,       0.1f, 0.f, 20.f);
	...
	ImGui::SeparatorText("Perlin Gust");
	{
		...
		ImGui::DragFloat("gust_speed_amp",     &g_wind.gust_speed_amp,  0.05f,  0.f,   4.f);
		ImGui::DragFloat("gust_lateral_boost", &gust_lateral_boost,     0.05f,  0.f,   6.f);
		ImGui::ProgressBar(g_wind.wind_gust_factor, ImVec2(-1, 0), "gust factor");
	}
}
