...
std::string write_transform3d(glm::mat4 transformIn) {
	...
	return "Transform3D(" +
		   std::string(string_format("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f", transform[0].x, transform[0].y,
									 transform[0].z, transform[1].x, transform[1].y, transform[1].z, transform[2].x,
									 transform[2].y, transform[2].z, transformIn[3].x, transformIn[3].y,
									 transformIn[3].z)) +
		   ")";
}
		...
		if (auto as_ent = obj->cast_to<Entity>()) {
			...
			if (model->get_num_lods() > 0 && model->has_lightmap_coords()) {
				...
				*/
				...
				out_scene_file += "[node name = \"mesh_" + std::to_string(mesh->get_instance_id()) +
								  "\" parent=\".\" instance=ExtResource(\"" + model_inst_id + "\")]\n";
...
shadow_enabled = true
	...
	for (auto model : models_to_export) {
		...
		std::string model_inst_id = StringUtils::alphanumeric_hash(model->get_name());
		header +=
			"[ext_resource type=\"PackedScene\" path=\"" + model_inst_id + ".glb\" id=\"" + model_inst_id + "\"]\n";
	}
...
Color32 get_color_of_material_for_export(const MaterialInstance* m) {
	...
	if (master->self->get_name() == "defaultPBR.mm") {
		...
		if (!c || c->type != MatParamType::Vector)
			return defaultc;
