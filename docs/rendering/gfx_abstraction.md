...
| Step | Scope |
| --- | --- |
| **P3.1** | **Toolchain spike — done.** `glslang` + `spirv-cross` installed as static libs (vcpkg, x64-windows), pragma-comment-linked from `Source/Render/SpirvCompile.{h,cpp}`. Source-loader lifted to `Source/Render/ShaderSourceLoader.{h,cpp}` (consumed by both backends). 5 round-trip tests in `test_spirv_pipeline.cpp` cross-check `IGraphicsShader::reflect()` vs SPIRV-Cross. **1/5 fully passes** (CullCompute); the other 4 surface real Phase 3 prep work — see [[gfx_abstraction_changelog]] → "Sub-phase P3.1 status" for the as-landed deliverable. Follow-ups: explicit `layout(location=N)` sweep on every shader stage I/O (P3.1.5); GL vs SPIRV-Cross sampler-count delta on VfogScatteringC (investigation); `<VcpkgApplocalDeps>true</>` on `App.vcxproj` (pre-existing B5 gap). |
