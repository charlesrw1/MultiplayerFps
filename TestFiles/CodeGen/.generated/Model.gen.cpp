// **** GENERATED SOURCE FILE version:1 ****
#include "./Model.h"
#include "Framework/ReflectionProp.h"
#include "Framework/ReflectionMacros.h"
#include "Game/AssetPtrMacro.h"
#include "Game/AssetPtrArrayMacro.h"
#include "Game/EntityPtrMacro.h"
#include "Scripting/FunctionReflection.h"
#include "Framework/VectorReflect2.h"
#include "Framework/EnumDefReflection.h"

PropertyInfoList Model::get_props()
{
	return {nullptr, 0};
}

ClassTypeInfo Model::StaticType = ClassTypeInfo(
                     "Model",
                     &IAsset::StaticType,
                     Model::get_props,
                     default_class_create<Model>(),
                     Model::CreateDefaultObject,
                     ""
                );
const ClassTypeInfo& Model::get_type() const{ return Model::StaticType;}
